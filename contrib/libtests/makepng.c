...
static int
find_parameters(png_const_charp what, png_const_charp cparam, png_charp *list,
   int nparams)
{
    ...
    */
   int i;
   png_charp param = strdup(cparam); // !memleak! quick hack to make const correctness work
...
static chunk_insert *
find_insert(png_const_charp what, png_const_charp param)
{
   png_uint_32 chunk = 0;
 ...
 */
static void
parse_color(const char *arg, unsigned int *colors)
{
   ...
   while (*arg && ncolors < 4)
   {
      ...
      unsigned long ul = strtoul(arg, (char **)&ep, 0);
...
#define main(cnt, arr)      png_makepng_main(cnt, arr)
#endif
